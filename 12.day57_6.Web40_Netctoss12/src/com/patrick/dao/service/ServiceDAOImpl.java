...
public class ServiceDAOImpl implements IServiceDAO {
	...
	public void pause(int id) throws DAOException {
		// TODO pause service method
	}
	
	
	@Override
	public List<Service> findByAccountId(int accountId) throws DAOException {
		...
		try {
			...
			e.printStackTrace();
			throw new DAOException("Fail to find service under this account id", e);
		}
	...
	private Service createService(ResultSet rs) throws SQLException{
		...
		sv.setUnixHost(rs.getString("unix_host"));
		sv.setOsUserName(rs.getString("os_username"));
		sv.setLoginPassword(rs.getString("login_passwd"));
		sv.setStatus(rs.getString("status"));
		sv.setCreateDate(rs.getDate("create_date"));
		sv.setCloseDate(rs.getDate("close_date"));
		sv.setCostId(rs.getInt("cost_id"));	
		return sv;	
	}
